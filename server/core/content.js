...
const getPage = function (req, res, listener) {
  return new Promise((resolve, reject) => {
    const page = core.config.onepager
    ...
    const check = (data) => {
      ...
      const isNoNamePage =
        core.template.cache.pages.indexOf(`${page}.html`) === -1;
      const isNoTypePage = data.doc
        ? core.template.cache.pages.indexOf(`${data.doc.type}.html`) === -1
        : true;
      ...
      if (isNoNamePage && isNoTypePage) {
        ...
        if (listener && listener.handlers.context) {
          listener.handlers.context(req, core.query.cache, context);
        }
    ...
    const render = (callback) => {
      ...
      core.template
        .render(core.config.template.layout, localObject)
        .then((html) => {
    ...
    const renderError = (callback, error) => {
      ...
      core.template
        .render(core.config.template.layout, { context: context })
        .then((html) => {
          callback(500, html);
        });
    };
