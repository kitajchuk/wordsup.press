...
const saveFile = (obj) => {
  return new Promise((resolve, reject) => {
    ...
    const _saveFile = () => {
      files
        .writeStr(place, obj.html)
        .then(() => {
          lager.cache(
            `[Clutch] Linking static link ${place.replace(process.cwd(), "")}`,
          );
    ...
    obj.html = String(obj.html).replace(
      regex,
      buildConfig.env.sandbox ? "" : buildConfig.url,
    );
    ...
    if (!obj.json.error) {
      ...
      files.write(placeJson, obj.json).then(() => {
        lager.cache(
          `[Clutch] Linking static link ${placeJson.replace(process.cwd(), "")}`,
        );
      ...
      const unlinkF = () => {
        ...
        if (fs.existsSync(link)) {
          ...
          if (regex.test(link)) {
            ...
            lager.error(
              `[Clutch] Un-Linking static link ${link.replace(process.cwd(), "")}`,
            );
            ...
            if (fs.existsSync(linkJson) && linkJson !== `${saveLoc}/`) {
              ...
              lager.error(
                `[Clutch] Un-Linking static link ${linkJson.replace(process.cwd(), "")}`,
              );
            }
          } else {
            ...
            .then(() => {
              if (options.create) {
                ...
                robots.generate().then((txt) => {
                  ...
                  files.writeStr(
                    fileSitemap,
                    xml.replace(baseUrl, buildConfig.url),
                    true,
                  );
                  ...
                  xmlJson.urlset.url.forEach((url) => {
                    requestApi(url)
                      .then((apiJson) => {
                        requestLoc(url)
                          .then((html) => {
                            saveFile({ html, url, json: apiJson })
                              .then(() => {
                                saved++;
